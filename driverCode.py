...
# 补齐指定长度数字
def getStrNo(no, lenth):
    return str(no).zfill(lenth)


# 生成年月日
def genBirth():
    return str(date(date.today().year - random.randrange(5, 70), 1, 1) + timedelta(days=random.randint(0, 364)))
...
# 生成姓名
def genName():
    count = random.sample([1, 2], 1)[0]
    return random.choice(lastName) + "".join(random.choice(firstName) for i in range(count))


def rdSample(list, num):
    return random.sample(list, num)[0]


# print(getSeq())         # 获取序号
# print(getIDCard())      # 获取可用身份证
# print(getPhoneNo())     # 获取可用手机号
print(getStrNo(1, 3))  # 补齐指定长度数字
print(genBirth())  # 生成年月日
print(genName())  # 生成姓名
print(rdSample(['a', 'b', 'c'], 1))